...
namespace xml_test{
    ...
    int choice;
    std::string file1 = "xml_test1.xml";
    std::string file2 = "xml_test2.xml";

    //读入两个数据，序列化到两个文件
    template <typename T, typename U>
    void write(T& obj1, U& obj2){

        xml_serialize(obj1, file1); 
        xml_serialize(obj2, file2); 
    }
    ...
    void load(T& obj){
        ...
        if (choice == 1)    xml_serialize_lib::xml_deserialize(obj, file1); 
        else if (choice == 2) xml_serialize_lib::xml_deserialize(obj, file2); 
    }
