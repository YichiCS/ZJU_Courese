#ifndef BASE64_SERIALIZE_H
#define BASE64_SERIALIZE_H
...
namespace base64_serialize_lib {
    ...
    struct TypeMapper<int> {
        static constexpr const char* type() { return "binary_int"; }
    };

    template <>
    struct TypeMapper<double> {
        static constexpr const char* type() { return "binary_double"; }
    };

    template <>
    struct TypeMapper<char> {
        static constexpr const char* type() { return "binary_char"; }
    };

    template <>
    struct TypeMapper<bool> {
        static constexpr const char* type() { return "binary_bool"; }
    };

    template <>
    struct TypeMapper<std::string> {
        static constexpr const char* type() { return "binary_string"; }
    };
    ...
    struct SerializeHelper {
        static void serialize(const T& obj, XMLDocument& doc, XMLElement* element){
            if constexpr (std::is_arithmetic_v<T>) {
                std::vector<char> bin_obj = to_binary(obj);
                std::string base64_obj = base64_encode(bin_obj);
                element->SetText(base64_obj.c_str());
            } else {
                obj.base64_serialize(doc, element);
            }
        }
        static void deserialize(T& obj, const XMLElement* element){
            if constexpr (std::is_arithmetic_v<T>) {
                std::string base64_obj = element->GetText();
                std::vector<char> bin_obj = base64_decode(base64_obj);
                obj = from_binary<T>(bin_obj);
            } else {
                obj.base64_deserialize(element);
            }
        }
    }; 
    //针对 bool
    ...
    struct SerializeHelper<bool> {
        static void serialize(const bool& obj, XMLDocument& doc, XMLElement* element) {
            std::string bool_string = obj ? "true" : "false";
            std::vector<char> bin_obj = to_binary(bool_string);
            std::string base64_obj = base64_encode(bin_obj);
            element->SetText(base64_obj.c_str());
        }
        static void deserialize(bool& obj, const XMLElement* element) {
            std::string base64_obj = element->GetText();
            std::vector<char> bin_obj = base64_decode(base64_obj);
            std::string bool_string = from_binary(bin_obj);
            obj = (bool_string.c_str() != nullptr && strcmp(bool_string.c_str(), "true") == 0);
        }
    };
    ...
    struct SerializeHelper<std::vector<T>> {
        static void serialize(const std::vector<T>& obj, XMLDocument& doc, XMLElement* element){
            std::vector<char> bin_size = to_binary(obj.size());
            std::string base64_size = base64_encode(bin_size);
            element->SetAttribute("size", base64_size.c_str()); 
        ...
        static void deserialize(std::vector<T>& obj, const XMLElement* element){
            obj.clear();
            std::string base64_size = element->Attribute("size"); 
            std::vector<char> bin_size = base64_decode(base64_size); 
            size_t size = from_binary<size_t>(bin_size);
            obj.resize(size);
    ...
    //通用接口
    template<typename T>
    void base64_serialize(const T& obj, const std::string& xmlFilePath) {
        XMLDocument doc;
    ...
    template<typename T>
    void base64_deserialize(T& obj, const std::string& xmlFilePath) {
        XMLDocument doc;
